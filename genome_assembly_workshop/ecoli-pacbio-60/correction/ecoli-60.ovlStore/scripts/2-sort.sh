...
#  This script should be executed from correction/ecoli-60.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./ecoli-60.ovlStore.BUILDING/$jobname.info -a ! -e ./ecoli-60.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './ecoli-60.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./ecoli-60.ovlStore.BUILDING \
  -S ../ecoli-60.seqStore \
  -C  ./ecoli-60.ovlStore.config \
  -f \
  -s $jobid \
  -M 1 
