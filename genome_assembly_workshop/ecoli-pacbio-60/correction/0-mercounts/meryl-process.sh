...
if [ -e ./ecoli-60.ms16.histogram -a -e ./ecoli-60.ms16.dump -a -e ./ecoli-60.ms16.ignore.gz ] ; then
  exit 0
fi
...
if [ -e ecoli-60.ms16.histogram ]; then
  exists1=true
else
  exists1=false
fi

if [ -e ecoli-60.ms16.dump ]; then
  exists2=true
else
  exists2=false
fi

if [ -e ecoli-60.ms16.ignore.gz ]; then
  exists3=true
else
  exists3=false
fi
if [ $exists1 = true -a $exists2 = true -a $exists3 = true ] ; then
  echo "Output files 'ecoli-60.ms16.histogram', 'ecoli-60.ms16.dump' and 'ecoli-60.ms16.ignore.gz' exist in 'correction/0-mercounts'."
  exit 0
fi
...
if [ ! -e ./ecoli-60.ms16/merylIndex ] ; then
  /Users/pfb15/canu-2.2/bin/meryl threads=4 memory=2 \
    greater-than 1 \
      output ecoli-60.ms16.WORKING \
      union-sum  \
        ./ecoli-60.01.meryl \
  && \
  mv -f ./ecoli-60.ms16.WORKING ./ecoli-60.ms16
  ...
  rm -rf ./ecoli-60.01.meryl ./ecoli-60.01.meryl.err
fi
...
if [ ! -e ./ecoli-60.ms16.histogram ] ; then
  /Users/pfb15/canu-2.2/bin/meryl threads=1 memory=1 \
    statistics ./ecoli-60.ms16 \
  > ./ecoli-60.ms16.histogram.WORKING \
  && \
  mv ./ecoli-60.ms16.histogram.WORKING ./ecoli-60.ms16.histogram
fi
...
if [ ! -e ./ecoli-60.ms16.dump ] ; then
  /Users/pfb15/canu-2.2/bin/meryl threads=4 memory=2 \
    print ./ecoli-60.ms16.##.dump \
      at-least threshold=72 \
      at-least word-frequency=0.0000001 \
        ./ecoli-60.ms16

  cat ./ecoli-60.ms16.??.dump > ./ecoli-60.ms16.dump
  rm -f ./ecoli-60.ms16.??.dump
fi
...
if [ ! -e ./ecoli-60.ms16.ignore.gz ] ; then
  numKmers=`wc -l < ./ecoli-60.ms16.dump`
  totKmers=`/Users/pfb15/canu-2.2/bin/meryl statistics ./ecoli-60.ms16 | grep present | awk '{ print $2 }'`


  ./meryl-make-ignore.pl $numKmers $totKmers < ./ecoli-60.ms16.dump | gzip -1c > ./ecoli-60.ms16.ignore.gz
fi
